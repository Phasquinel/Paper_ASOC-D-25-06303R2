...
//*******************************************************************************************//
decl res5,Menvelope9, res5_gen,res5q_inf,res5q_sup,Res9qq,res5q_r,res5q_min,res5q_mean,res5q_max;
   ...
   //********************** E N V E L O P E *********************************************************************//        
    ...
    //turn off warnings
    oxwarning( 0 );
   	// selection of generator type “George Marsaglia”
 	ranseed("GM");
	ranseed({1965, 2001}); 
        ...
        fprint(fpout,"\n\t\t |-------  BETA REGRESSION MODEL -------|" );                       
		 ...
		 data = loadmat("CCA_OX_PROX_0.txt"); // USE THE DATA AVAILABLE TO RUN THE CODE 
		...
		fprint(fpout,"median s_vy",quantilec(sortc(s_vy), <0.50>));
 ...
 //************* Maximum likelihood estimation *****************//
	  ...
	  if(ir1 == MAX_CONV || ir1 == MAX_WEAK_CONV)
	  {
	    ...
	    decl K_theta = m_XAumentada'*m_Q*m_XAumentada; 
...
/*************************DEFINING THE PROJECTION MATRIX FOR Z */
		...
		hV1 = diagonal(HstarV1)';
			  ...
			  decl Temp_inv   = invertsym(s_mZ'*Vi*s_mZ); 
			  decl L1         = invertsym(s_mX'*m_phih*Q*s_mX - s_mX'*F*T*H*s_mZ*Temp_inv*s_mZ'*H'*T'*F'*s_mX);
		      decl L2         = -L1*s_mX'*F*T*H*s_mZ*Temp_inv;
		      decl L3         = Temp_inv*(unit(kk2)+ s_mZ'*H'*T'*F'*s_mX*L1*s_mX'*F*T*H*s_mZ*Temp_inv);
		      Obsinver        = ((L1~L2)|(L2'~L3));
	 		  B1              = ( XQXinv~zeros(kk1,kk2) )|( zeros(kk2,kk1)~zeros(kk2,kk2));
 		      B2              = (zeros(kk1,kk1)~zeros(kk1,kk2))|(zeros(kk2,kk1)~Temp_inv );
			  Lbeta           =  Obsinver - B2;
			  Lphi            =  Obsinver - B1;
	  
		      E            = diag(ystar-mustar);
		      A            = diag(va);
			  DeltaP       = (s_mX'*m_phih*T*E)|s_mZ'*H*A;
		      LF_P         = DeltaP'*Obsinver*DeltaP;
			  LF_P_beta    = DeltaP'*Lbeta*DeltaP;
			  LF_P_phi     = DeltaP'*Lphi*DeltaP;
		      eigensym(LF_P, &meval, &mevec);
			  C_maxP       = meval[0];
			  I_maxP       = mevec[][0];
              eigensym(LF_P_beta, &meval, &mevec);
			  C_maxPbeta   = meval[0];
			  I_maxPbeta   = mevec[][0];
    		  eigensym(LF_P_phi, &meval, &mevec);
			  C_maxPphi    = meval[0];
			  I_maxPphi    = mevec[][0];
		   	  CP_t         = 2*diagonal(LF_P);
			  CP_beta_t    = 2*diagonal(LF_P_beta);
			  CP_phi_t     = 2*diagonal(LF_P_phi);
			  println("\nCases ", C_maxP );
			   ...
			   Sy          = diag(sqrt((muhat.*(1-muhat))./(1+phihat)));
			   DeltaR      = ((s_mX'*m_phih*T*M*Sy)|(s_mZ'*H*B*Sy));
			   LF_R        = DeltaR'*Obsinver*DeltaR;
			   LF_R_beta   = DeltaR'*Lbeta*DeltaR;
			   LF_R_phi    = DeltaR'*Lphi*DeltaR;
		       eigensym(LF_R, &meval, &mevec);
			   C_maxR      = meval[0];
			   I_maxR      = mevec[][0];
               eigensym(LF_R_beta, &meval, &mevec);
			   C_maxRbeta  = meval[0];
			   I_maxRbeta  = mevec[][0];
    	   	   eigensym(LF_R_phi, &meval, &mevec);
			   C_maxRphi   = meval[0];
			   I_maxRphi   = mevec[][0];
			   CR_t        = 2*diagonal(LF_R);
        	   CR_beta_t   = 2*diagonal(LF_R_beta);
	           CR_phi_t    = 2*diagonal(LF_R_phi);
			   srtotal     = 2*sumr(CR_t)/cn;         
			   srbeta      = 2*sumr(CR_beta_t)/cn;    
			   srphi       = 2*sumr(CR_phi_t)/cn;      
			   println("\nResponse Influence ", C_maxR );
			 

			  decl L_y   = (s_mX'*m_phih*T*M)|(s_mZ'*H*B);
			  decl D_mu  = T*s_mX~zeros(cn,kk2);
			  decl V_GL  = D_mu*Obsinver*L_y;
...
//********************************************************** Calculation of quantities for constructing envelope bands **********************************************************//                                                                  
			 ...
			 Menvelope9 = zeros(cn,80);				 
				   ...
				   Res8qq[][2] = res8_max ; 
						 ...
						 Ajuste[0][] = pseudoR2LR; 	    
						 Ajuste[1][] = pseudoR2LRc;      
						 Ajuste[2][] = pseudoR2;	    Ajuste[4][] = AIC;
						 Ajuste[3][] = pseudoR2c;	    Ajuste[5][] = BIC;
...
//    /* Data, Hora e  Tempo de ExecuÃ§Ã£o */											 //    /* Data, Hora e  Tempo de ExecuÃ§Ã£o */
